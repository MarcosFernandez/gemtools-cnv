...
void gt_map_calculate_template_mapq_score(gt_template *template,gt_map_score_attributes *ms_attr)
{
  ...
  uint64_t min_score[6]={0xffff,0xffff,0xffff,0xffff,0xffff,0xffff};
  {
	...
	if(maps[rd]) {
	  ...
	  if(!mp_hash) {
	    ...
	    if(!maps[rd^1]) {
	      if(seq_like[rd]<min_score[rd+4]) min_score[rd+4]=seq_like[rd];
	      unpd[rd]=true;
	    }
	  }
	}
  ...
  if(unpaired) {
    // For paired reads, the best aligning read that we didn't find would be the best *unpaired* read found from one end combined
  ...
  // And now we have to enter the MAPQ values in the map structures
  {
    GT_TEMPLATE_ITERATE_MMAP__ATTR_(template,maps,maps_attr) {
	  ...
	  HASH_FIND(hh,mhash[rd],buf,key_size,mp_hash);
	  assert(mp_hash);
	  GT_MAP_ITERATE(maps[rd],mapb) { mapb->phred_score=mapb->se_phred_score=mp_hash->phred; }
	}
      if(maps[0] && maps[1]) {
	size_t key_size=gt_map_score_make_pair_key(buf,maps[0],maps[1]);
	HASH_FIND(hh,mhash[2],buf,key_size,mp_hash);
	assert(mp_hash);
	// We say that a paired end MAPQ can not be more than the sum of the single end scores
	uint64_t zsc=maps[0]->phred_score+maps[1]->phred_score;
	if(mp_hash->phred>zsc) mp_hash->phred=zsc;
	if(mp_hash->phred1>zsc) mp_hash->phred1=zsc;
...
void gt_map_calculate_alignment_mapq_score(gt_alignment *alignment,gt_map_score_attributes *ms_attr)
{
  ...
  if(nmaps) {
	...
	if(tp>254) tp=254;
      ...
      GT_MAP_ITERATE(maplist[k].map,mapb) { mapb->phred_score=mapb->se_phred_score=tp;	}
    }
    free(maplist);
  }
}
