...
def _install_bundle(install_dir, base=None):
    ...
    # copy CNV tools
    bins = [x for x in os.listdir("GEMTools-cnv/bin")]
    for file in bins:
        if not file.endswith("gz") and file != "gt.construct":
            print "Copy binary library: %s to %s" % (file, install_dir)
            result_file = "%s/%s" % (install_dir, file)
            if os.path.exists(result_file):
                os.remove(result_file)
            shutil.copy("%s/%s" % ("GEMTools-cnv/bin", file), install_dir)
            os.chmod(result_file, 0755)


    # copy CNV tools
    bins = [x for x in os.listdir("R")]
    for file in bins:
        print "Copy script: %s to %s" % (file, install_dir)
        result_file = "%s/%s" % (install_dir, file)
        if os.path.exists(result_file):
            os.remove(result_file)
        shutil.copy("%s/%s" % ("R/", file), install_dir)
        os.chmod(result_file, 0755)
...
def compile_gemtools_cnv():
        process = subprocess.Popen(['make'], shell=True, cwd='GEMTools-cnv')
        if process.wait() != 0:
            print >> sys.stderr, """

Error while compiling GEMTools-cnv. That is very unfortunate.
...
setup(
        ...
        name='Gemtools-cnv',
        version=__VERSION__,
        description='Python support library for the GEM mapper and the gemtools library to perform copy number calls',
        author='Marcos Fernandez Callejo, Santiago Marco Sola,Thasso Griebel',
        author_email='mfernandez@pcb.ub.es',
...
It also provides a set of tools to perform copy number variance calls in whole genome shotgun data.
...
#                "matplotlib==1.2.0"
        ...
        entry_points={
            'console_scripts': [
                'gemtools-cnv = gem.commands:gemtools'
            ]
        },
)
