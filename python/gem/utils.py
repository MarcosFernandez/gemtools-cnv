...
def runFindCommand(path,pattern):
    '''Return find linux command'''
    findCommand = ""
    if pattern == "":
        findCommand += "find " + path + " -type f"
    else:
        findCommand += 'find ' + path + " -name \""+ pattern + "\""
    return  subprocess.Popen(findCommand, stdout=subprocess.PIPE, shell=True).stdout.read()